...
import { generateAgentResponse, renderMarkdown } from "./helpers";
import type { ChatMessage } from "./types";

interface AgentPanelProps {
  chatMessages: ChatMessage[];
  onAddMessage: (content: string, role: "user" | "agent") => void;
}

export default function AgentPanel({ chatMessages, onAddMessage }: AgentPanelProps) {
  ...
  const handleSend = useCallback(() => {
    ...
    setChatInput("");
    onAddMessage(msg, "user");
    setTimeout(() => onAddMessage(generateAgentResponse(msg), "agent"), 800);
  }, [chatInput, onAddMessage]);

  const handleQuickCmd = useCallback((cmd: string) => {
    onAddMessage(cmd, "user");
    setTimeout(() => onAddMessage(generateAgentResponse(cmd), "agent"), 800);
  }, [onAddMessage]);
  ...
  const toggleVoice = useCallback(() => {
    ...
    if (!SR) { setVoiceStatus("Reconhecimento de voz não suportado"); return; }
    const rec = new SR();
    rec.lang = "pt-PT";
    rec.continuous = false;
    rec.interimResults = true;
    recognitionRef.current = rec;
    rec.onstart = () => { setIsRecording(true); setVoiceStatus("A ouvir... Fale o seu comando"); };
    rec.onresult = (e: any) => {
      const t = e.results[0][0].transcript;
      setVoiceStatus(t);
      if (e.results[0].isFinal) {
        onAddMessage(t, "user");
        setTimeout(() => onAddMessage(generateAgentResponse(t), "agent"), 800);
        setIsRecording(false);
        setVoiceStatus("Clique no microfone para falar com o Eng. Marcos");
      }
    };
    rec.onerror = () => { setIsRecording(false); setVoiceStatus("Erro. Tente novamente."); };
    rec.onend = () => { setIsRecording(false); };
    rec.start();
  }, [isRecording, onAddMessage]);

  const quickCommands = ["Verificar cotas", "Colisões de redes", "Gerar relatório", "Terra × Fund.", "Normas PT", "Materiais"];
