...
interface EngSilvaPanelProps {
  ...
  /** IncompatiCheck context — when provided, chat has obra-specific intelligence */
  incompaticheckContext?: {
...
const QUICK_PROMPTS_INCOMPATICHECK = [
  ...
  { label: '📊 Resumo', prompt: 'Faz um resumo geral da análise' },
];
...
const QUICK_PROMPTS_GENERAL = [
  ...
  { label: '⚠️ Não-Conformidades', prompt: 'Como classificar uma não-conformidade em obra?' },
...
export function EngSilvaPanel({
  ...
  // Determine which messages/functions to use
  const messages = isIncompatiCheck && incompaticheckContext
    ? incompaticheckContext.chatMessages.map(m => ({
        id: m.id,
        role: m.role,
        content: m.content,
  ...
  const isThinking = isIncompatiCheck && incompaticheckContext
    ? incompaticheckContext.agentThinking
    : generalThinking;
  ...
  // Page context label
  const getPageLabel = () => {
    if (isIncompatiCheck) return incompaticheckContext?.obraName || 'IncompatiCheck';
  ...
  // General chat send (outside IncompatiCheck)
  ...
  const handleSend = useCallback(async () => {
    ...
    setSending(true);
    try {
      if (isIncompatiCheck && incompaticheckContext) {
        await incompaticheckContext.sendUserMessage(text);
      } else {
  ...
  }, [input, sending, isIncompatiCheck, incompaticheckContext, sendGeneralMessage]);
  ...
  const handleQuickPrompt = useCallback(async (prompt: string) => {
    ...
    try {
      if (isIncompatiCheck && incompaticheckContext) {
        await incompaticheckContext.sendUserMessage(prompt);
      } else {
  ...
  }, [sending, isIncompatiCheck, incompaticheckContext, sendGeneralMessage]);
          ...
          <div className="flex flex-col items-center justify-center py-8 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1 max-w-[280px]">
                Engenheiro civil sénior com experiência em fiscalização de obras em Portugal.
                Posso ajudar com normas, incompatibilidades, materiais e pareceres técnicos.
              </p>
            </div>
